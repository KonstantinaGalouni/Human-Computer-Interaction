...
<html>
	<head>
		<title>Εύδοξος: Σύνθετη Αναζήτηση</title>
		<meta charset="UTF-8">
		<link rel="stylesheet" type="text/css" href="css/advanced_search_connected.css" />
		...
		<script type="text/javascript">
			...
			function OnClickHelp(event) {
				alert("Συμπληρώστε τα κουτιά τα οποία επιθυμείτε και θεωρείται σχετικά με την αναζήτηση\nΣτη συνέχεια πιέστε το Enter, ή το κουμπί Search δεξιά του κουτιού\nΓια απλούστερη αναζήτηση μεταβείτε στην Απλή Αναζήτηση, πατώντας το κουμπί αριστερά\n\n(*Σημείωση: Στο τελευταίο κουτί της σύνθετης αναζήτησης δε συμπληρώνετε, αλλά επιλέγεται την κατάλληλη κατηγορία)");
			}
	...
	<body onkeypress="GetChar (event);">
		...
		<div id="main">
			...
			<div id = "navbar">
				<a href="index_connected.php" style="text-decoration:none;">Αρχική</a> > Σύνθετη Αναζήτηση
			</div>
			...
			<div id="right">
				...
				<form method="post" action="advanced_search_results.php"> 
					<table id="search_fields">
						<tr>
						  <td><input type="text" name="Title" value="Τίτλος..." onclick="OnClick(event)"></td>
						  <td><input type="text" name="Subtitle" value="Υπότιτλος..." onclick="OnClick(event)"></td>
						  <td><input type="text" name="Code" value="Κωδικός Βιβλίου..." onclick="OnClick(event)"></td>
						  <td><input type="text" name="Writers" value="Συγγραφείς..." onclick="OnClick(event)"></td>
						</tr>
						<tr>
						  <td><input type="text" name="PublNum" value="Αριθμός Έκδοσης..." onclick="OnClick(event)"></td>
						  <td><input type="text" name="PubYear" value="Έτος Έκδοσης..." onclick="OnClick(event)"></td>
						  <td><input type="text" name="VolTitle" value="Τίτλος Τόμου..." onclick="OnClick(event)"></td>
						  <td><input type="text" name="VolNum" value="Αριθμός Τόμου..." onclick="OnClick(event)"></td>
						</tr>
						<tr>
						  <td><input type="text" name="KeyWds" value="Λέξεις Κλειδιά..." onclick="OnClick(event)"></td>
						  <td><input type="text" name="ISBN" value="ISBN..." onclick="OnClick(event)"></td>
						  <td><input type="text" name="Publisher" value="Διαθέτης(Εκδότης)..." onclick="OnClick(event)"></td>
						  <td><input type="text" name="Edition" value="Εκδόσεις..." onclick="OnClick(event)"></td>
						</tr>
						<tr>
						  <td><input type="text" name="School" value="Σχολή/Τμήμα..." onclick="OnClick(event)"></td>
						  <select name="Type" >
							<option value="writing">Σύγγραμμα(Προεπιλ.)</option>
							<option value="ewriting">Ηλεκτρονικό Σύγγραμμα</option>
							<option value="ebook">Ηλεκτρονικό Βιβλίο</option>
							<option value="free">Δωρεάν ηλεκτρονικό Βοήθημα/Σημειώσεις</option>
						  </select>
						</tr>
					</table>
					
					<div class="search_field2">
						<button id="submit_register" type="submit" style="border: 2px solid blue; background-color:#F2F2F2;"> <font size="5">Αναζήτηση </font></button>
					</div>
				</form>
				
				<button onclick="OnClickHelp(event)" id="help" type="button">?	Βοήθεια</button>
				<a href="simple_search_connected.php"><button id="alternate_search" type=button> Απλή Αναζήτηση</button></a>
			</div>
		</div>
